...
int main(int argc, char* argv[]){
  ...
  if(my_rank == 0){
    ...
    ans_q = bfs(nap_size,object,table_size);
    //show_queue(&ans_q);
    for(dest = 2; dest < 16 ; dest++){
      begin_data = dequeue(&ans_q);
    ...
    //takaoka = linear_relaxation(tmp,strobj *object,int index,int table_size,int value)
